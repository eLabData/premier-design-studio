...
import { Send, Upload, Sparkles, RotateCcw, Smartphone, Type, X, ImagePlus } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Textarea } from "@/components/ui/textarea";
import { useToast } from "@/hooks/use-toast";
import type { StoreMessage, StoreMetadata } from "@/types/logo";

export type { StoreMessage, StoreMetadata };

interface StoreAssetsChatProps {
  onNewImage: (url: string) => void;
  onStoreData: (data: StoreMetadata) => void;
  isLoading: boolean;
  setIsLoading: (v: boolean) => void;
  messages: StoreMessage[];
  setMessages: React.Dispatch<React.SetStateAction<StoreMessage[]>>;
}

const StoreAssetsChat = ({
  onNewImage,
  onStoreData,
  isLoading,
  setIsLoading,
  ...
  const sendMessage = async (mode: "mockup" | "metadata" | "all" = "all") => {
    ...
    const userMsg: StoreMessage = {
      ...
      content:
        text ||
        (mode === "mockup"
          ? "Create store screenshot mockups from this image"
          : mode === "metadata"
          ? "Generate App Store & Google Play metadata"
          : "Generate store assets from this"),
    ...
    try {
      ...
      const assistantMsg: StoreMessage = {
        id: crypto.randomUUID(),
        role: "assistant",
        content: data.text || "Aqui estão seus assets!",
      ...
      if (data.storeData) onStoreData(data.storeData);
    } catch (err: unknown) {
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-secondary/30">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-foreground">Store Assets</span>
          {messages.length > 0 && (
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 min-h-[300px] max-h-[500px]">
        {messages.length === 0 && (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-muted-foreground mb-1">App Store & Google Play</p>
            <p className="text-xs text-muted-foreground/70">
              Envie screenshots, descreva seu app, e receba assets, metadados e mockups prontos para a loja.
            </p>
          </div>
        )}
        ...
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
            ...
            >
              ...
              {msg.storeData && (
                <div className="mt-2 p-2 rounded-lg bg-background/50 text-xs space-y-1">
                  {msg.storeData.appName && (
                    <p>
                      <span className="text-muted-foreground">Nome:</span> {msg.storeData.appName}
                    </p>
                  )}
                  {msg.storeData.subtitle && (
                    <p>
                      <span className="text-muted-foreground">Subtítulo:</span> {msg.storeData.subtitle}
                    </p>
                  )}
                  {msg.storeData.category && (
                    <p>
                      <span className="text-muted-foreground">Categoria:</span> {msg.storeData.category}
                    </p>
                  )}
                  {msg.storeData.keywords && (
                    <p>
                      <span className="text-muted-foreground">Palavras-chave:</span> {msg.storeData.keywords}
                    </p>
                  )}
                </div>
              )}
          ...
          <div className="flex justify-start">
            <div className="bg-secondary rounded-xl px-4 py-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground">Gerando assets…</span>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="border-t border-border p-3 space-y-2">
        <Textarea
          ...
          placeholder="Descreva seu app, ou arraste screenshots aqui…"
...
export default StoreAssetsChat;
